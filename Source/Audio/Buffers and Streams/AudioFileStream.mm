...
void AudioFileStream::setEffectParameter(int effectPosition, int parameterID, float value)
{
    ...
    {
        if (value < 0.5f) {
             m_pbBypassStateArray.set(effectPosition, false);
        } else {
             m_pbBypassStateArray.set(effectPosition, true);
        }
    }
...
float AudioFileStream::getEffectParameter(int effectPosition, int parameterID)
{
    if (parameterID == PARAM_BYPASS)
    {
        if (m_pbBypassStateArray.getUnchecked(effectPosition)) {
            return 1.0f;
        } else {
            return 0.0f;
        }
...
void AudioFileStream::motionUpdate(float *motion)
{
    ...
    {
        m_fGain = motion[ATTITUDE_PITCH];
        transportSource.setGain(m_pcParameter.getUnchecked(PARAM_MOTION_GAIN)->process(m_fGain * m_fGain) * GAIN_SCALE);
    }
...
float AudioFileStream::getMotionParameter(int effectPosition, int parameterID)
{
    return audioEffectSource.getUnchecked(effectPosition)->getMotionParameter(parameterID);
}


void AudioFileStream::setPlayheadPosition(float position)
{
    transportSource.setPosition(position * (m_iSampleLength / m_fSampleRate));
}
