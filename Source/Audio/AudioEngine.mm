...
//    liveAudioStream     =   new AudioStream();
    ...
    masterRecordingPath         = File::getSpecialLocation(File::userDocumentsDirectory).getFullPathName() + "/MasterRecording.wav";
    userRecordingLibraryPath    = File::getSpecialLocation(File::userDocumentsDirectory).getFullPathName() + "/UserRecordings";
    
    File userRecordingLibrary   = File(userRecordingLibraryPath);
    if (!userRecordingLibrary.exists())
    {
        userRecordingLibrary.createDirectory();
    }
    
    recordingFilePathArray1.clear();
    recordingFilePathArray2.clear();
    playbackFilePathArray.clear();
    buttonPagePosition.clear();
    
    masterRecordingToggle = false;
...
void AudioEngine::setButtonPagePosition(int sampleID, int position)
{
    buttonPagePosition.set(sampleID, position);
}

int AudioEngine::getButtonPagePosition(int sampleID)
{
    return buttonPagePosition.getUnchecked(sampleID);
}
...
void AudioEngine::setCurrentFXPackIndex(int index)
{
    m_iCurrentFXPackLoaded = index;
}

void AudioEngine::setCurrentSampleBankIndex(int index)
{
    m_iCurrentPresetBankLoaded = index;
}

int AudioEngine::getCurrentSampleBankIndex()
{
    return m_iCurrentPresetBankLoaded;
}

int AudioEngine::getCurrentFXPackIndex()
{
...
//--- Audio Recordings from Microphone ---//
...
void AudioEngine::startRecordingMaster()
{
    audioMixer->startRecordingOutput(masterRecordingPath);
...
void AudioEngine::stopRecordingMaster()
{
...
void AudioEngine::saveCurrentRecording(String filename)
{
    File currentFile = File (userRecordingLibraryPath + "/" + filename + ".wav");
    File(masterRecordingPath).copyFileTo(currentFile);
}

int AudioEngine::getNumberOfUserRecordings()
{
    File dir = File(userRecordingLibraryPath);
    return (dir.findChildFiles(userRecordingFiles, File::findFiles, false));
}

String AudioEngine::getUserRecordingFileName(int index)
{
    return userRecordingFiles.getUnchecked(index).getFileNameWithoutExtension();
}

void AudioEngine::loadUserRecordedFile(int sampleID, int index)
{
    loadAudioFile(sampleID, userRecordingFiles.getUnchecked(index).getFullPathName());
}
...
int AudioEngine::getTempo()
{
    return audioMixer->getTempo();
}
...
float AudioEngine::getMotionParameter(int sampleID, int effectPosition, int parameterID)
{
    return audioMixer->getMotionParameter(sampleID, effectPosition, parameterID);
}


void AudioEngine::setPlayheadPosition(int sampleID, float position)
{
    audioMixer->setPlayheadPosition(sampleID, position);
}